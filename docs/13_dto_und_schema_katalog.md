...
| id | uuid |
| type | enum |
| status | enum(pending, queued, running, completed, partial_failure, failed, cancelled) |
...
| requestType | string |
| priority | enum |
| status | enum(open, triaged, assigned, in_progress, waiting_user, escalated, resolved, closed) |
...
| status | enum(open, preparing, submitted, under_review, followup_required, escalated, removed, partially_removed, rejected, closed) |
