...
export const osintQueryTypeSchema = z.enum(['username', 'email', 'phone', 'domain', 'ip', 'name', 'image_url']);
export const osintSearchScopeSchema = z.enum(['quick', 'comprehensive']);
export const osintSearchStatusSchema = z.enum(['queued', 'running', 'completed', 'failed']);
export const osintResultSchemaVersionSchema = z.literal('1.0');

export const osintSourceSchema = z.object({
...
export const osintTimestampsSchema = z.object({
  discoveredAt: isoDateTimeSchema.optional(),
  collectedAt: isoDateTimeSchema.optional(),
  updatedAt: isoDateTimeSchema,
});

export const osintResultItemSchema = z.object({
  ...
  sources: z.array(osintSourceSchema).default([]),
  evidence: z.record(z.string(), z.unknown()).default({}),
  timestamps: osintTimestampsSchema.default({ updatedAt: '2026-01-01T00:00:00.000Z' }),
  normalized: z.record(z.string(), z.unknown()).default({}),
}).superRefine((value, ctx) => {
  if (!value.source && value.sources.length === 0) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['sources'],
      message: 'Mindestens eine Quelle mit tool-Angabe ist erforderlich.',
    });
  }
});

export const osintUnifiedResultSchema = z.object({
  schemaVersion: osintResultSchemaVersionSchema,
  searchId: z.string().trim().min(1),
  status: osintSearchStatusSchema,
  query: z.string().trim().min(1),
  queryType: osintQueryTypeSchema,
  scope: osintSearchScopeSchema,
  confidence: z.number().min(0).max(1).default(0),
  confidenceScore: z.number().min(0).max(1).default(0),
  providers: z.array(z.string().trim().min(1)).default([]),
  sources: z.array(osintSourceSchema).default([]),
  evidence: z.record(z.string(), z.unknown()).default({}),
  timestamps: osintTimestampsSchema,
  results: z.array(osintResultItemSchema).default([]),
});

export const createOsintSearchRequestSchema = z.object({
  query: z.string().trim().min(1).max(2048),
  queryType: osintQueryTypeSchema,
  scope: osintSearchScopeSchema.default('quick'),
  providers: z.array(z.string().trim().min(1)).max(20).default([]),
  idempotencyKey: optionalTrimmedStringSchema,
}).superRefine((value, ctx) => {
  if (value.queryType === 'email' && !value.query.includes('@')) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['query'],
      message: 'Query must be a valid email-like value for queryType=email',
    });
  }
});
...
export type CreateOsintSearchRequest = z.infer<typeof createOsintSearchRequestSchema>;
export type OsintUnifiedResult = z.infer<typeof osintUnifiedResultSchema>;
...
export function safeParseCreateOsintSearchRequest(input: unknown): ContractValidationResult<CreateOsintSearchRequest> {
  const parsed = createOsintSearchRequestSchema.safeParse(input);
...
export function safeParseOsintUnifiedResult(input: unknown): ContractValidationResult<OsintUnifiedResult> {
  const parsed = osintUnifiedResultSchema.safeParse(input);
...
export function parseCreateOsintSearchRequest(input: unknown) { return createOsintSearchRequestSchema.parse(input); }
export function parseOsintUnifiedResult(input: unknown) { return osintUnifiedResultSchema.parse(input); }
...
export const userAvatarUploadIntentRequestSchema = z.object({
...
export const workerQueueNameSchema = z.enum(['assets', 'checks', 'osint', 'reviews', 'support', 'removals', 'retention']);
...
export const osintExecuteJobPayloadSchema = z.object({
  searchId: z.string().trim().min(1),
  query: z.string().trim().min(1).max(2048),
  queryType: osintQueryTypeSchema,
  scope: osintSearchScopeSchema,
  ...
  }).default({ type: 'exponential', delayMs: 30000 }),
  requestedBy: optionalTrimmedStringSchema,
});
...
export const bullmqJobEnvelopeSchema = z.discriminatedUnion('name', [
  ...
  bullmqJobBaseSchema.extend({
    queue: z.literal('osint'),
    name: z.literal('osint.execute'),
    payload: osintExecuteJobPayloadSchema,
  }),
  bullmqJobBaseSchema.extend({
...
export type UserAvatarUploadIntentRequest = z.infer<typeof userAvatarUploadIntentRequestSchema>;
...
export type OsintExecuteJobPayload = z.infer<typeof osintExecuteJobPayloadSchema>;
...
export function safeParseChangePasswordRequest(input: unknown): ContractValidationResult<ChangePasswordRequest> {
...
export function safeParseUserAvatarUploadIntentRequest(input: unknown): ContractValidationResult<UserAvatarUploadIntentRequest> {
  const parsed = userAvatarUploadIntentRequestSchema.safeParse(input);
