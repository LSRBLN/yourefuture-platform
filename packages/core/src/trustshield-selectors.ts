import type {
  ...
  ReviewQueueItem,
  SearchFilterState,
  SupportQueueFilterState,
  SupportQueueItem,
  UnifiedSearchRecord,
} from './trustshield-records.js';
import type { ReviewDecisionOutcome, ReviewPriority, SlaRisk, SortDirection } from './status.js';

function normalizeFilterQuery(query: string) {
  return query.trim().toLowerCase();
}

function matchesFilterQuery(query: string, values: Array<string | undefined>) {
...
function matchesArrayFilter<T extends string>(selectedValues: T[], candidateValue: T | undefined) {
...
function matchesStringListFilter(selectedValues: string[], candidateValue: string | undefined) {
  if (selectedValues.length === 0) {
    return true;
  }
...
function compareText(left: string | undefined, right: string | undefined, direction: SortDirection) {
  const result = (left ?? '').localeCompare(right ?? '', undefined, { sensitivity: 'base' });
  return direction === 'asc' ? result : result * -1;
}

function comparePriority(left: ReviewPriority, right: ReviewPriority, direction: SortDirection) {
  const priorityRank: Record<ReviewPriority, number> = { low: 0, medium: 1, high: 2, urgent: 3 };
  const result = priorityRank[left] - priorityRank[right];
  return direction === 'asc' ? result : result * -1;
}

function compareSlaRisk(left: SlaRisk, right: SlaRisk, direction: SortDirection) {
  const slaRiskRank: Record<SlaRisk, number> = { healthy: 0, watch: 1, risk: 2, breach: 3 };
...
export function selectOutcomeActionStates(items: ReviewQueueItem[], outcomes: ReviewDecisionOutcome[]) {
  return outcomes.reduce<Record<string, OutcomeActionState>>((accumulator, outcome) => {
...
export function matchesSearchFilter(record: UnifiedSearchRecord, filterState: SearchFilterState) {
  return (
    matchesFilterQuery(filterState.query, [record.id, record.title, record.platform, record.summary, record.assignedTo, record.nextActionLabel]) &&
...
export function selectSearchRecords(records: UnifiedSearchRecord[], filterState: SearchFilterState) {
...
export function matchesRemovalFilter(record: RemovalCaseRecord, filterState: RemovalFilterState) {
  return (
    matchesFilterQuery(filterState.query, [record.id, record.platform, record.targetUrl, record.legalBasis, record.assignedTo, record.nextActionLabel]) &&
...
export function selectRemovalCases(records: RemovalCaseRecord[], filterState: RemovalFilterState) {
...
export function matchesReviewQueueFilter(item: ReviewQueueItem, filterState: ReviewQueueFilterState) {
  return (
    matchesFilterQuery(filterState.query, [
      item.id,
      item.summary,
...
export function selectReviewQueueItems(items: ReviewQueueItem[], filterState: ReviewQueueFilterState) {
...
export function matchesSupportQueueFilter(item: SupportQueueItem, filterState: SupportQueueFilterState) {
  return (
    matchesFilterQuery(filterState.query, [item.id, item.requesterLabel, item.note, item.assignedTo, item.linkedRemovalCaseId, item.linkedReviewItemId]) &&
    matchesArrayFilter(filterState.priorities, item.priority) &&
...
export function selectSupportQueueItems(items: SupportQueueItem[], filterState: SupportQueueFilterState) {
...
export function getSearchFilterSummary(filterState: SearchFilterState) {
  return {
    hasActiveFilters:
      Boolean(filterState.query) || filterState.statuses.length > 0 || filterState.reviewPriorities.length > 0 || filterState.evidenceCoverage.length > 0 || filterState.platforms.length > 0 || filterState.assignedTo.length > 0 || filterState.onlyEscalated || !filterState.includeSensitive,
    activeFilterCount:
      (filterState.query ? 1 : 0) + filterState.statuses.length + filterState.reviewPriorities.length + filterState.evidenceCoverage.length + filterState.platforms.length + filterState.assignedTo.length + (filterState.onlyEscalated ? 1 : 0) + (filterState.includeSensitive ? 0 : 1),
  };
}

export function getRemovalFilterSummary(filterState: RemovalFilterState) {
  return {
    hasActiveFilters:
      Boolean(filterState.query) || filterState.statuses.length > 0 || filterState.severities.length > 0 || filterState.evidenceCoverage.length > 0 || filterState.slaRisks.length > 0 || filterState.platforms.length > 0,
    activeFilterCount:
      (filterState.query ? 1 : 0) + filterState.statuses.length + filterState.severities.length + filterState.evidenceCoverage.length + filterState.slaRisks.length + filterState.platforms.length,
  };
}

export function getReviewQueueFilterSummary(filterState: ReviewQueueFilterState) {
  return {
    hasActiveFilters:
...
export function getSupportQueueFilterSummary(filterState: SupportQueueFilterState) {
  return {
    hasActiveFilters:
      Boolean(filterState.query) || filterState.priorities.length > 0 || filterState.requestTypes.length > 0 || filterState.statuses.length > 0 || filterState.assignedTo.length > 0 || filterState.slaRisks.length > 0,
    activeFilterCount:
      (filterState.query ? 1 : 0) + filterState.priorities.length + filterState.requestTypes.length + filterState.statuses.length + filterState.assignedTo.length + filterState.slaRisks.length,
  };
}
