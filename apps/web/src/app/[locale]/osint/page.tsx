      ...
      <div className="sticky top-0 z-50 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">{t('osint.dashboard')}</h1>
              <p className="text-sm text-slate-400">{t('nav.osint')}</p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-8">
          <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
            <p className="text-slate-400 text-sm">{t('osint.searchTypes')}</p>
            <p className="text-white font-semibold mt-1">{t('osint.username')} • {t('osint.email')} • {t('osint.phone')} • {t('osint.domain')} • {t('osint.image')}</p>
          </div>
          <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
            <p className="text-slate-400 text-sm">{t('osint.integratedSources')}</p>
            <p className="text-white font-semibold mt-1">10+ OSINT Tools & Databases</p>
          </div>
          <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
            <p className="text-slate-400 text-sm">{t('osint.responseTime')}</p>
        ...
        <div className="mt-8 bg-slate-800/50 border border-slate-700 rounded-lg p-6">
          <h3 className="text-white font-semibold mb-3">💡 {t('osint.tips')}</h3>
          <ul className="space-y-2 text-slate-300 text-sm">
            <li>• <strong>Multi-Source:</strong> {t('osint.search')} will query all 10+ sources simultaneously</li>
