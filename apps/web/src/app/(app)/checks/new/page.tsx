...
import { defaultSearchFilterState, selectSearchRecords } from '@trustshield/core';
...
export default function NewCheckPage() {
  ...
  const intakeLinkedRecords = selectSearchRecords([] as Parameters<typeof selectSearchRecords>[0], {
  ...
  const apiStatusLabel =
    apiSubmissionState === 'idle'
      ? t('api.status.idle')
      : apiSubmissionState === 'ready'
        ? t('api.status.ready')
        : apiSubmissionState === 'submitting'
          ? t('api.status.submitting')
          : t('api.status.submitted');
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(18rem,0.85fr)]">
        <div className="space-y-6">
          <Card tone="muted" className="space-y-5">
            ...
            <Stepper
              items={[
                { label: t('progress.steps.concern.label'), description: t('progress.steps.concern.description'), state: 'complete' },
                { label: t('progress.steps.intake.label'), description: t('progress.steps.intake.description'), state: 'current' },
                { label: t('progress.steps.legal.label'), description: t('progress.steps.legal.description'), state: 'upcoming' },
              ]}
            />
          </Card>
          ...
          <Card tone="muted" className="space-y-5">
            <div className="space-y-2">
              <SectionLabel>{t('concern.label')}</SectionLabel>
              <h2 className="font-[family-name:var(--font-heading)] text-2xl leading-tight text-stone-950">
                {t('concern.title')}
              </h2>
            </div>
              ...
              {concernOptions.map((option) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {intakeState.concern === option.id ? <Badge className="bg-[#094cb2] text-white">{t('concern.active')}</Badge> : null}
                  </div>
          ...
          <Card className="space-y-8">
            <div className="space-y-2">
              ...
              {showIntakeErrors ? (
                <IntakeErrorPanel title={t('errors.panelTitle')} summary={t('errors.panelSummary')} items={intakeErrorItems} />
              ) : null}
            </div>
            ...
            <Card tone="muted" className="space-y-5">
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <FeatureCard className="space-y-3 bg-[#f8f5ef]">
                  ...
                  {intakeLinkedRecords[0] ? <SearchEvidenceCard record={intakeLinkedRecords[0]} /> : null}
              </div>
              ...
              <Card tone="muted" className="space-y-4">
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  <Button variant="secondary" onClick={simulateApiPreparation}>{t('api.validateDraft')}</Button>
                  <Button onClick={simulateSubmit} disabled={!apiReady || apiSubmissionState === 'submitting'}>{apiSubmissionState === 'submitting' ? t('api.submitSending') : apiSubmissionState === 'submitted' ? t('api.submitConfirmed') : t('api.submitStart')}</Button>
                </div>
              </Card>
            </Card>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                 <Button variant="secondary" onClick={() => hydrateConcernTemplate(intakeState.concern)}>{t('actions.resumeLater')}</Button>
                 <Button disabled={!canContinue}>{t('actions.nextLegal')}</Button>
               </div>
             </div>
          </Card>
        </div>
