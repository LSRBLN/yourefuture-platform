...
export function useUploadAsset(options: UseUploadAssetOptions = {}) {
  ...
  const upload = async (file: File): Promise<string | null> => {
    try {
      ...
      const payload = await requestApi<UploadIntentEnvelope>(
      ...
      const intent: UploadIntentResponse = {
        ...
        presignedUrl: payload.upload?.uploadUrl ?? '',
        headers: payload.upload?.headers,
      };

      if (!intent.assetId || !intent.presignedUrl) {
      ...
      const error = new Error(getApiErrorMessage(err, 'Asset upload failed'));
      setError(error);
