...
import { SecureAssetStorageService } from '../../modules/assets/secure-storage.js';
...
export class NestAssetsService {
  ...
  async createUploadIntent(body: unknown, context: ApiRequestContext) {
    ...
    const quarantineObjectKey = `${actorSubject}/${assetId}/${validation.data.originalFilename.replace(/\s+/g, '-').toLowerCase()}`;
    ...
    const quarantineStorageKey = `quarantine/${quarantineObjectKey}`;
