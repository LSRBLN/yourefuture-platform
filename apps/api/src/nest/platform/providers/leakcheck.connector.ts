...
export class LeakCheckConnector implements ILeakProvider {
  name = 'LeakCheck';
  enabled: boolean;
  private readonly logger = new Logger(LeakCheckConnector.name);
  ...
  async searchUsername(username: string): Promise<BreachResult[]> {
    if (!this.enabled) {
      this.logger.warn('Connector disabled');
      return [];
    }
    ...
    try {
      ...
      this.logger.error('Error searching username', error instanceof Error ? error.stack : String(error));
      throw error;
    }
  }
  ...
  async searchPhone(phone: string): Promise<BreachResult[]> {
    void phone;
    this.logger.log('Phone search may not be available on all plans');
    return [];
  }
  ...
  async searchDomain(domain: string): Promise<BreachResult[]> {
    ...
    try {
      ...
      this.logger.error('Error searching domain', error instanceof Error ? error.stack : String(error));
      throw error;
    }
  }
